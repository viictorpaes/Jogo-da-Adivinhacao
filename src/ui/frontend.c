...
void desenhar_menu_principal(void) {
    ...
    Botao btn_adivinhacao = desenhar_botao(400, 350, 400, 80, "1. Jogo da Adivinhação");
    Botao btn_memoria = desenhar_botao(400, 480, 400, 80, "2. Jogo da Memória");
    Botao btn_sair = desenhar_botao(400, 610, 400, 80, "3. Sair");
    
    if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        if (btn_adivinhacao.hover) {
            // Próximo estado
        } else if (btn_memoria.hover) {
            // Próximo estado
...
void desenhar_menu_dificuldade(void) {
    ...
    Botao btn_facil = desenhar_botao(250, 250, 700, 100, 
                                     "FÁCIL (1-50, 10 tentativas)");
    Botao btn_medio = desenhar_botao(250, 400, 700, 100, 
                                     "MÉDIO (1-100, 7 tentativas)");
    Botao btn_dificil = desenhar_botao(250, 550, 700, 100, 
...
void desenhar_jogo_adivinhacao(const EstadoUI *ui) {
    ...
    int text_width = MeasureText(ui->entrada_texto, 40);
    ...
    Botao btn_confirmar = desenhar_botao(200, 480, 400, 60, "CONFIRMAR PALPITE (Enter)");
    
    // Instruções
    DrawText("Pressione ESC para voltar ao menu", 200, 600, 20, COR_TEXTO);
}
...
void desenhar_resultado_adivinhacao(const EstadoUI *ui) {
    ...
    snprintf(resultado, sizeof(resultado), 
             "Número Secreto: %d\nTentativas Usadas: %d de %d",
             ui->partida_atual.numero_secreto,
             ui->partida_atual.tentativas_usadas,
             ui->partida_atual.max_tentativas);
    DrawText(resultado, 300, 380, 25, COR_TEXTO);
    
    Botao btn_voltar = desenhar_botao(400, 550, 400, 80, "Voltar ao Menu (ESC)");
}
...
void desenhar_resultado_memoria(const EstadoUI *ui) {
    ...
    snprintf(stats, sizeof(stats),
             "Pontuação Total: %d\nPares Encontrados: %d/8\nTentativas: %d\nEficiência: %.1f%%",
             ui->jogo_memoria.pontuacao,
             ui->jogo_memoria.pares_encontrados,
             ui->jogo_memoria.tentativas,
             (ui->jogo_memoria.pares_encontrados * 2.0f / ui->jogo_memoria.tentativas) * 100.0f);
    
    DrawText(stats, 300, 280, 30, COR_TEXTO);
    
    Botao btn_voltar = desenhar_botao(400, 600, 400, 80, "Voltar ao Menu (ESC)");
}
    ...
    if (ui->estado_atual == ESTADO_JOGANDO_ADIVINHACAO) {
        ...
        if (IsKeyPressed(KEY_ENTER) && ui->indice_entrada > 0) {
            ...
            if (ui->entrada_numero >= ui->partida_atual.min_range && 
                ...
                Resultado resultado = processar_palpite(&ui->partida_atual, ui->entrada_numero);
