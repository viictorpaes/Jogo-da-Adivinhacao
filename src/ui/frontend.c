...
static void desenhar_timer(double secs, double max_secs, float x, float y, float largura)
{
    ...
    char texto_timer[32];
    snprintf(texto_timer, sizeof(texto_timer), "Tempo: %.0fs", secs < 0 ? 0.0 : secs);
    DrawText(texto_timer, (int)x, (int)y, 24, cor);
...
static void desenhar_timer_generico(double secs, double max_secs, float x, float y, float largura)
{
    Color cor = cor_timer(secs / max_secs);
    char texto_timer[32];
    snprintf(texto_timer, sizeof(texto_timer), "%.0fs", secs < 0 ? 0.0 : secs);
    DrawText(texto_timer, (int)x, (int)y, 22, cor);
...
static void salvar_partida_adivinhacao(const EstadoUI *ui)
{
    ...
    reg_adivinhacao.dificuldade = ui->dificuldade_selecionada;
    reg_adivinhacao.tentativas_usadas = ui->partida_atual.tentativas_usadas;
    reg_adivinhacao.max_tentativas = ui->partida_atual.max_tentativas;
    reg_adivinhacao.numero_secreto = ui->partida_atual.numero_secreto;
    reg_adivinhacao.venceu = ui->partida_atual.venceu;
    reg_adivinhacao.pontos = calcular_pontos(ui->dificuldade_selecionada, ui->partida_atual.tentativas_usadas, ui->partida_atual.venceu);

    salvar_partida(&reg_adivinhacao);
}
...
static void data_hoje(char *buf, int len)
{
    time_t agora = time(NULL);
    struct tm *tempo_local = localtime(&agora);
    strftime(buf, len, "%Y-%m-%d", tempo_local);
}
...
void desenhar_inserir_nome(const EstadoUI *ui)
{
    ...
    const char *titulo = is_p2 ? "IDENTIFICAÇÃO DO ASTRONAUTA 2" : "IDENTIFICAÇÃO DO ASTRONAUTA 1";
    ...
    {
        int cursor_x = (int)(caixa.x+16)+MeasureText(nome_atual,38);
        DrawText("|",cursor_x,(int)(caixa.y+12),38,COR_PRIMARIA);
    }
    ...
    if (ui->nomes_hist_n > 0)
    {
        ...
        for (int ni = 0; ni < ui->nomes_hist_n && n_matches < 6; ni++)
        {
            ...
            strncpy(nome_hist_lower, ui->nomes_hist[ni], 63);
            nome_hist_lower[63] = '\0';

            for (int k = 0; nome_hist_lower[k]; k++) 
            {
                nome_hist_lower[k] = (char)tolower((unsigned char)nome_hist_lower[k]);
            }

            if (!nome_lower[0] || strstr(nome_hist_lower, nome_lower)) 
            {
                matches[n_matches++] = ni;
            }
        }

        if (n_matches > 0)
        {
            float dropdown_x = caixa.x;
...
void desenhar_menu_dificuldade(EstadoUI *ui)
{
    ...
    if (ui->modo_pendente == MODO_LOGICA) 
    { 
        titulo = "PROTOCOLO LóGICO - PATENTE";
    }

    if (ui->modo_pendente == MODO_PRECEDENCIA) 
    { 
        titulo = "HIERARQUIA DE CMDS - PATENTE";
    }
...
void desenhar_jogo_adivinhacao(const EstadoUI *ui)
{
    ...
    {
        int distancia = ui->entrada_numero - ui->partida_atual.numero_secreto;
        ...
        int percentual_distancia = range > 0 ? distancia * 100 / range : 100;
        const char *msg; Color cor;

        if (percentual_distancia < 5)
        {
            msg = "Sinal estabelecido! Frequência muito próxima!";
            cor = COR_SUCESSO;
        }

        else if (percentual_distancia < 15)
        {
            msg = "Sinal detectado! Continue ajustando...";
            cor = COR_SECUNDARIA;
        }
        
        else if (percentual_distancia < 40)
        {
            msg = "Interferência estática... Sinal fraco.";
            cor = COR_SECUNDARIA;
        }

        else
        {
            msg = "Sem sinal no espaço... Muito longe!";
            cor = COR_ERRO;
        }
...
void desenhar_jogo_vs(const EstadoUI *ui)
{
    ...
    char placar_vs[128];
    snprintf(placar_vs,sizeof(placar_vs),"Rodada %d/%d | %s: %d rodada(s) | %s: %d rodada(s)", ui->vs_rodada+1, VS_MAX_RODADAS, ui->nome_jogador,  ui->vs_rodadas_vencidas[0], ui->nome_jogador2, ui->vs_rodadas_vencidas[1]);
    ...
    {
        int distancia = ui->entrada_numero - ui->vs_partida.numero_secreto;
    ...
    DrawText(tempo_j1,          870,362,18,col_j1);
    DrawText(ui->nome_jogador2, 590,400,20,col_j2);
    DrawText(tempo_j2,          870,402,18,col_j2);

    if (ui->vs_jogador_atual == 0)
    {
        DrawText("<",1050,362,20,COR_SUCESSO);
    }

    else
    {
        DrawText("<",1050,402,20,COR_AVISO);
    }
}
...
void desenhar_jogo_memoria(const EstadoUI *ui)
{
    ...
    if (bar_cheia > bar_largura) 
    {  
        bar_cheia = bar_largura;
    }
    ...
    DrawRectangle(840, 98, (int)bar_largura, 8, COR_BOTAO);
    DrawRectangle(840, 98, (int)bar_cheia,   8, cor_barra_timer);

    _desenhar_grade_memoria(&ui->jogo_memoria, 200, 145, 80, 8, 4, 4, ui->clique_casa1, ui->clique_casa2);
...
void desenhar_jogo_memoria_vs(const EstadoUI *ui)
{
    ...
    _desenhar_grade_memoria(&ui->mem_vs_jogo, 280, 145, 80, 8, 4, 4, ui->mem_vs_clique1, ui->mem_vs_clique2);
...
void desenhar_jogo_logica(const EstadoUI *ui)
{
    ...
    snprintf(header,sizeof(header),"Questão %d/%d | Acertos VF:%d | Acertos Class:%d | Pts:%d", j->questoes_respondidas+1, j->questoes_total, j->acertos_vf, j->acertos_class, calcular_pontos_logica(j));

    DrawText(header,60,72,18,COR_TEXTO);

    double tempo_maximo = j->fase==FASE_FEEDBACK ? 2.5 : j->tempo_por_questao;
    double tempo_atual = j->fase==FASE_FEEDBACK ? j->timer_feedback : j->timer;

    desenhar_timer_generico(tempo_atual,tempo_maximo,860,72,270);
    ...
    for (int v = 0; v < j->qtd_vars; v++)
    {
        ...
        snprintf(fragmento_var,sizeof(fragmento_var),"%c = %s  ","PQR"[v],j->vars[v]?"V":"F");
        strcat(texto_valores,fragmento_var);
    }

    DrawText(texto_valores,60,200,22,COR_TEXTO);
    ...
    {
        char texto_resposta_vf[64];

        snprintf(texto_resposta_vf,sizeof(texto_resposta_vf),"Você respondeu: %s  |  Correto: %s", j->resp_vf==1?"V":"F",j->resp_correta_vf?"V":"F");

        Color cor_resposta_vf = j->acertou_vf ? COR_SUCESSO : COR_ERRO;

        DrawText(texto_resposta_vf,60,265,20,cor_resposta_vf);
    ...
    else
    {
        ...
        char texto_feedback_classif[128];

        snprintf(texto_feedback_classif,sizeof(texto_feedback_classif),"Classificação: %s  (correto: %s)", j->resp_class>=0?classes[j->resp_class]:"---",classes[j->classificacao]);

        DrawText(texto_feedback_classif,60,308,22,j->acertou_class?COR_SUCESSO:COR_ERRO);
    ...
    DrawText("TAUTOLOGIA = sempre V  |  CONTRADIÇÃO = sempre F  |  CONTINGÊNCIA = V ou F dependendo dos valores", 60,516,14,COR_TEXTO);
}
...
void desenhar_jogo_precedencia(const EstadoUI *ui)
{
    ...
    snprintf(cabecalho,sizeof(cabecalho),"Questão %d/%d | Acertos: %d | Pts: %d", j->questoes_respondidas+1, j->questoes_total, j->acertos, calcular_pontos_prec(j));

    DrawText(cabecalho,60,72,18,COR_TEXTO);

    double tempo_atual  = j->mostrando_feedback ? j->timer_feedback : j->timer;
    double tempo_maximo = j->mostrando_feedback ? 2.0 : j->tempo_por_questao;
    ...
    for (int k = 0; k < 4; k++) 
    {
        ...
        {
            if (k == j->questao.correta) 
            {
                cor_btn = (Color){0,120,60,255};
            }

            else if (k == j->resp_selecionada && !j->acertou) 
            {
                cor_btn = (Color){120,0,40,255};
            }
        }
...
void desenhar_resultado_adivinhacao(const EstadoUI *ui)
{
    ...
    snprintf(linha_patente,    sizeof(linha_patente),    "Patente    : %s", nome_patente);
    ...
    const char *dica=heuristica_adivinhacao(ui->partida_atual.tentativas_usadas, ui->partida_atual.max_tentativas, ui->partida_atual.venceu,  ui->dificuldade_selecionada);
    ...
    if (ui->pode_avancar_fase)
    {
        ...
        int largura_texto = MeasureText(buf_av, 22);

        DrawText(buf_av, 640 + 225 - largura_texto/2, 497, 22, COR_SUCESSO);
...
void desenhar_resultado_vs(const EstadoUI *ui)
{
    ...
    int vitorias_j1 = ui->vs_rodadas_vencidas[0];
    int vitorias_j2 = ui->vs_rodadas_vencidas[1];

    const char *nome_vencedor = vitorias_j1 > vitorias_j2 ? ui->nome_jogador : vitorias_j2 > vitorias_j1 ? ui->nome_jogador2 : "EMPATE";

    Color cor_vencedor = vitorias_j1 > vitorias_j2 ? COR_SUCESSO : vitorias_j2 > vitorias_j1 ? COR_AVISO : (Color){200, 200, 200, 255};

    char placar[128];

    snprintf(placar, sizeof(placar), "VENCEDOR: %s", nome_vencedor);
    DrawText(placar, 380, 108, 32, cor_vencedor);

    char linha_j1[64], linha_j2[64];

    snprintf(linha_j1, sizeof(linha_j1), "%s: %d rodada(s) | %d pts", ui->nome_jogador,  vitorias_j1, ui->vs_pontos[0]);
    snprintf(linha_j2, sizeof(linha_j2), "%s: %d rodada(s) | %d pts", ui->nome_jogador2, vitorias_j2, ui->vs_pontos[1]);
...
void desenhar_resultado_memoria_vs(const EstadoUI *ui)
{
    ...
    const char *nome_vencedor = pts_j1 > pts_j2 ? ui->nome_jogador : pts_j2 > pts_j1 ? ui->nome_jogador2 : "EMPATE";

    Color cor_vencedor = pts_j1 > pts_j2 ? COR_SUCESSO : pts_j2 > pts_j1 ? COR_AVISO : (Color){200, 200, 200, 255};

    char placar[128];

    snprintf(placar, sizeof(placar), "VENCEDOR: %s", nome_vencedor);
    DrawText(placar, 400, 108, 30, cor_vencedor);
...
void desenhar_resultado_logica(const EstadoUI *ui)
{
    ...
    if (ui->pode_avancar_fase)
    {
        ...
        char buf_permanecer[64], buf_avanc_fase[64];

        snprintf(buf_permanecer, sizeof(buf_permanecer), "Permanecer: %s", nome_patente);
        snprintf(buf_avanc_fase,   sizeof(buf_avanc_fase),   "Avancar: %s  →", proxima_patente);

        desenhar_botao(100, 355, 450, 58, buf_permanecer);

        DrawRectangle(640, 355, 450, 58, (Color){30,80,30,255});
        DrawRectangleLines(640, 355, 450, 58, COR_SUCESSO);
        int largura_texto = MeasureText(buf_avanc_fase, 22);
        DrawText(buf_avanc_fase, 640 + 225 - largura_texto/2, 374, 22, COR_SUCESSO);
...
void desenhar_resultado_precedencia(const EstadoUI *ui)
{
    ...
    if (ui->pode_avancar_fase)
    {
        ...
        desenhar_botao(100, 315, 450, 58, buf_permanecer);
        ...
        DrawText(buf_avanc_fase, 640 + 225 - largura_texto/2, 334, 22, COR_SUCESSO);
...
void desenhar_historico(EstadoUI *ui)
{
    ...
    {
        char msg_filtro[96];
        snprintf(msg_filtro, sizeof(msg_filtro), "Astronauta: %s  (ESC = historico geral)", filtro);
        DrawText(msg_filtro, 50, 63, 15, COR_AVISO);
...
#define X1 20
    ...
    {
        int y=topo+40; 
        bool tem_registros=false;

        for (int i=ui->hist_adiv_n-1; i>=0 && y<318; i--)
        {
            RegistroPartida *entrada_adiv=&ui->hist_adiv[i];

            if (filtro && strstr(entrada_adiv->nome, filtro)==NULL) continue;
    ...
    DrawLine(X1,354,375,354,COR_SECUNDARIA);
    {
        ...
        {
            RegistroVS *entrada_vs=&ui->hist_vs[i];

            if (filtro && strstr(entrada_vs->nome1,filtro)==NULL && strstr(entrada_vs->nome2,filtro)==NULL) continue;
...
#define X2 405
    ...
    DrawLine(X2,topo+36,780,topo+36,COR_SECUNDARIA);
    {
        ...
        for (int i=ui->hist_mem_n-1; i>=0 && y<318; i--)
        {
            ...
            if (filtro && strstr(entrada_mem->nome, filtro)==NULL) continue;

            tem_registros=true;

            char linha[100];

            snprintf(linha,sizeof(linha),"%s %-10s %djog %dpts",
                     entrada_mem->data,entrada_mem->nome,entrada_mem->tentativas,entrada_mem->pontos);
    ...
    DrawLine(X2,354,780,354,COR_SECUNDARIA);
    {
        ...
        for (int i=ui->hist_mem_vs_n-1; i>=0 && y<560; i--)
        {
            ...
            if (filtro && strstr(entrada_mem_vs->nome1,filtro)==NULL && strstr(entrada_mem_vs->nome2,filtro)==NULL) continue;

            tem_registros=true;

            const char *nome_vencedor = entrada_mem_vs->pontos1>entrada_mem_vs->pontos2?entrada_mem_vs->nome1:entrada_mem_vs->pontos2>entrada_mem_vs->pontos1?entrada_mem_vs->nome2:"Empate";
...
#define X3 795
    ...
    DrawLine(X3,topo+20,1170,topo+20,COR_SECUNDARIA);
    {
        ...
        for (int i=ui->hist_logica_n-1; i>=0 && y<318; i--)
        {
            ...
            if (filtro && strstr(entrada_logica->nome, filtro)==NULL) continue;
    ...
    DrawLine(X3,354,1170,354,COR_SECUNDARIA);
    {
        ...
        for (int i=ui->hist_prec_n-1; i>=0 && y<560; i--)
        {
            ...
            if (filtro && strstr(entrada_prec->nome, filtro)==NULL) continue;
            tem_registros=true;
...
static void processar_menu_principal(EstadoUI *ui)
{
    ...
    for (int i = 0; i < N_ITENS_MENU; i++)
    {
        Rectangle item_rect = {ITENS_MENU[i].x, ITENS_MENU[i].y, ITENS_MENU[i].w, ITENS_MENU[i].h};

        if (IsKeyReleased(TECLAS_MENU[i]) || clique_em_rect(item_rect))
        {
            escolha = i + 1;
            break;
        }
...
{
    if (!nome || !nome[0] || ui->nomes_hist_n >= 100) 
    {   
        return;
    }
    ...
    {
        if (strcmp(ui->nomes_hist[j], nome) == 0) 
        {
            return;
        }
    }
...
static void carregar_nomes_historico(EstadoUI *ui)
{
    ...
    RegistroPartida buf_adiv[50]; 
    int n_adiv = carregar_historico(buf_adiv, 50);

    for (int i = 0; i < n_adiv; i++) 
    {
        _add_nome_unico(ui, buf_adiv[i].nome);
    }

    RegistroMemoria buf_mem[50];  
    int n_mem = carregar_historico_memoria(buf_mem, 50);

    for (int i = 0; i < n_mem; i++) 
    { 
        _add_nome_unico(ui, buf_mem[i].nome);
    }

    RegistroVS buf_vs[50];   
    int n_vs = carregar_historico_vs(buf_vs, 50);

    for (int i = 0; i < n_vs; i++)
    {
        _add_nome_unico(ui, buf_vs[i].nome1);
        _add_nome_unico(ui, buf_vs[i].nome2);
    }

    RegistroMemoriaVS buf_mvs[50]; 
    int n_mvs = carregar_historico_memoria_vs(buf_mvs, 50);

    for (int i = 0; i < n_mvs; i++)
    {
        _add_nome_unico(ui, buf_mvs[i].nome1);
        _add_nome_unico(ui, buf_mvs[i].nome2);
    }

    RegistroPuzzle 
    buf_log[50]; 
    int n_log = carregar_historico_puzzle(buf_log, 50, "logica");

    for (int i = 0; i < n_log; i++) 
    {
         _add_nome_unico(ui, buf_log[i].nome);
    }

    RegistroPuzzle buf_prec[50]; 
    int n_prec = carregar_historico_puzzle(buf_prec, 50, "precedencia");

    for (int i = 0; i < n_prec; i++) 
    {
        _add_nome_unico(ui, buf_prec[i].nome);
    }

    ui->nomes_hist_carregados = true;
}

static bool processar_clique_autocomplete(EstadoUI *ui, char *nome, int *idx)
{
    if (!IsMouseButtonReleased(MOUSE_LEFT_BUTTON)) return false;

    Rectangle caixa = {130, 225, 580, 65};
    float dropdown_x = caixa.x, dropdown_y = caixa.y + caixa.height, dropdown_largura = caixa.width;

    char nome_lower[64];
    strncpy(nome_lower, nome, 63);
    nome_lower[63] = '\0';
...
static void processar_nome(char *nome, int *idx, int max_len)
{
    ...
    if (IsKeyReleased(KEY_BACKSPACE) && *idx > 0) 
    {
        nome[--(*idx)] = '\0';
    }
}
...
void processar_entrada(EstadoUI *ui)
{
    switch (ui->estado_atual)
    {
        ...
        case ESTADO_INSERIR_NOME_P2:
        {
            ...
            bool clicou_sugestao = processar_clique_autocomplete(ui, ui->nome_jogador2, &ui->nome_indice2);
    ...
    case ESTADO_DIFICULDADE:
    {
        ...
        if (IsKeyReleased(KEY_TWO) || clique_em_rect((Rectangle){200,380,800,95}))
        {
            dificuldade_escolhida = MEDIO;
            escolheu = true;
        }

        if (IsKeyReleased(KEY_THREE) || clique_em_rect((Rectangle){200,530,800,95}))
        {
        ...
        if (escolheu)
        {
            ui->dificuldade_selecionada = dificuldade_escolhida;
            ...
            {
                ui->partida_atual    = iniciar_partida(dificuldade_escolhida);
                ui->partida_salva    = false;
                ui->timer_adiv       = timer_adiv_para_dif(dificuldade_escolhida);
            ...
            {
                ui->vs_partida              = iniciar_partida(dificuldade_escolhida);
                ...
                ui->vs_salvo                = false;
                ui->vs_timer[0]             = timer_adiv_para_dif(dificuldade_escolhida);
                ui->vs_timer[1]             = timer_adiv_para_dif(dificuldade_escolhida);
                ui->estado_atual            = ESTADO_JOGANDO_VS;
            }

            else if (ui->modo_pendente == MODO_LOGICA)
            {
                ui->logica       = inicializar_jogo_logica(dificuldade_escolhida);
            ...
            {
                ui->precedencia       = inicializar_jogo_precedencia(dificuldade_escolhida);
    ...
    case ESTADO_JOGANDO_VS:
    {
        if (ui->vs_rodada_acabou)
        {
            if (IsKeyReleased(KEY_ENTER) || IsKeyReleased(KEY_ESCAPE))
            {
                if (ui->vs_jogo_acabou)
                {
                    ...
                    {
                        RegistroVS reg_vs;
                        data_hoje(reg_vs.data,sizeof(reg_vs.data));
                        strncpy(reg_vs.nome1,ui->nome_jogador,63);
                        strncpy(reg_vs.nome2,ui->nome_jogador2,63);
                        reg_vs.dificuldade = ui->dificuldade_selecionada;
                        reg_vs.vitorias1   = ui->vs_rodadas_vencidas[0];
        ...
        int jogador_ativo = ui->vs_jogador_atual;
        int key = GetCharPressed();
        while (key > 0)
        {
            if (key >= '0' && key <= '9' && ui->indice_entrada < 9)
        ...
        if (IsKeyReleased(KEY_ENTER) || clique_em_rect((Rectangle){60,460,420,60}))
        {
            ...
            else
            {
                ...
                else
                {
                    ui->entrada_numero = palpite;
                    memset(ui->mensagem_erro, 0, sizeof(ui->mensagem_erro));
                    ui->vs_tentativas[jogador_ativo]++;
                    Resultado resultado_palpite = processar_palpite(&ui->vs_partida, palpite);
                    memset(ui->entrada_texto, 0, sizeof(ui->entrada_texto));
                    ui->indice_entrada = 0;

                    if (resultado_palpite == ACERTOU)
                    {
                        ui->vs_timer[jogador_ativo] += 15.0;
                    }

                    else
                    {
                        ui->vs_timer[jogador_ativo] -= 5.0;

                        if (ui->vs_timer[jogador_ativo] < 0.0) 
                        { 
                            ui->vs_timer[jogador_ativo] = 0.0;
                        }
                    }

                    if (resultado_palpite == ACERTOU)
                    {
                        ui->vs_rodadas_vencidas[jogador_ativo]++;
                        ui->vs_pontos[jogador_ativo] += calcular_pontos(ui->dificuldade_selecionada, ui->vs_tentativas[jogador_ativo], true);
                    ...
                    else
                    {
                        ...
                        {
                            int proximo_jogador = (jogador_ativo + 1) % 2;

                            if (ui->vs_tentativas[proximo_jogador] >= VS_MAX_TENTATIVAS) 
                            {
                                proximo_jogador = jogador_ativo;
                            }

                            ui->vs_jogador_atual = proximo_jogador;
                        }
                    }
                }
            }
    ...
    case ESTADO_RESULTADO_MEMORIA_VS:
    {
        if (!ui->mem_vs_salvo) 
        {
            RegistroMemoria reg_j1, reg_j2;
            data_hoje(reg_j1.data, sizeof(reg_j1.data));
            data_hoje(reg_j2.data, sizeof(reg_j2.data));
            strncpy(reg_j1.nome, ui->nome_jogador, 63);
            strncpy(reg_j2.nome, ui->nome_jogador2, 63);
            reg_j1.pontuacao  = ui->mem_vs_pares[0];
            reg_j2.pontuacao  = ui->mem_vs_pares[1];
            reg_j1.tentativas = reg_j2.tentativas = ui->mem_vs_jogo.tentativas;
            reg_j1.pontos     = ui->mem_vs_pontos[0];
            reg_j2.pontos     = ui->mem_vs_pontos[1];
            salvar_partida_memoria_vs(&reg_j1, &reg_j2);
    ...
    case ESTADO_RESULTADO_LOGICA:
    {
        if (!ui->logica_salva) 
        {
            RegistroPuzzle reg_logica;
            data_hoje(reg_logica.data, sizeof(reg_logica.data));
            strncpy(reg_logica.nome, ui->nome_jogador, 63);
            reg_logica.acertos = ui->logica.acertos_vf + ui->logica.acertos_class;
            reg_logica.total   = ui->logica.questoes_total * 2;
            reg_logica.pontos  = calcular_pontos_logica(&ui->logica);
            strncpy(reg_logica.modo, "logica", sizeof(reg_logica.modo) - 1);
            salvar_puzzle(&reg_logica);
    ...
    case ESTADO_RESULTADO_PRECEDENCIA:
    {
        ...
        {
            RegistroPuzzle reg_precedencia;
            data_hoje(reg_precedencia.data, sizeof(reg_precedencia.data));
            strncpy(reg_precedencia.nome, ui->nome_jogador, 63);
            reg_precedencia.acertos = ui->precedencia.acertos;
            reg_precedencia.total   = ui->precedencia.questoes_total;
            reg_precedencia.pontos  = calcular_pontos_prec(&ui->precedencia);
            strncpy(reg_precedencia.modo, "precedencia", sizeof(reg_precedencia.modo) - 1);
            salvar_puzzle(&reg_precedencia);
...
void atualizar_ui(EstadoUI *ui)
{
    ...
    {
        int jogador_ativo = ui->vs_jogador_atual;
        ui->vs_timer[jogador_ativo] -= dt;

        if (ui->vs_timer[jogador_ativo] <= 0.0)
        {
            ui->vs_timer[jogador_ativo] = 0.0;
            int outro_jogador = 1 - jogador_ativo;
            bool outro_sem_tent  = ui->vs_tentativas[outro_jogador] >= VS_MAX_TENTATIVAS;
            bool outro_sem_tempo = ui->vs_timer[outro_jogador] <= 0.0;
            ...
            {
                ui->vs_jogador_atual = outro_jogador;
                memset(ui->entrada_texto,0,sizeof(ui->entrada_texto));
                ui->indice_entrada=0;
                memset(ui->mensagem_erro,0,sizeof(ui->mensagem_erro));
            }
    ...
    if (ui->estado_atual == ESTADO_JOGANDO_MEMORIA_VS && !ui->mem_vs_aguardando) 
    {
        ...
        ui->mem_vs_timer[jogador_ativo] -= dt;

        if (ui->mem_vs_timer[jogador_ativo] <= 0.0)
        {
            ui->mem_vs_timer[jogador_ativo] = 0.0;
            ui->mem_vs_jogador = (jogador_ativo + 1) % 2;
            ui->mem_vs_clique1 = 0;
            ui->mem_vs_clique2 = 0;
        }
...
void desenhar_ui(EstadoUI *ui)
{
    switch (ui->estado_atual)
    {
        ...
        case ESTADO_JOGANDO_MEMORIA_VS:
            desenhar_jogo_memoria_vs(ui);
            break;

        case ESTADO_JOGANDO_LOGICA:
            desenhar_jogo_logica(ui);
            break;

        case ESTADO_JOGANDO_PRECEDENCIA:
            desenhar_jogo_precedencia(ui);
            break;
        ...
        case ESTADO_RESULTADO_MEMORIA_VS:
            desenhar_resultado_memoria_vs(ui);
            break;

        case ESTADO_RESULTADO_LOGICA:
            desenhar_resultado_logica(ui);
            break;

        case ESTADO_RESULTADO_PRECEDENCIA:
            desenhar_resultado_precedencia(ui);
            break;
