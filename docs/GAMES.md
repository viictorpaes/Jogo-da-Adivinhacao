...
| **Versus (1v1)** | Adivinhação VS, Memória VS |

Todos os modos estão disponíveis nas duas interfaces:

| Modo | Console (`make run`) <br> <img src="https://img.shields.io/badge/-🕹️%20Terminal-020617?style=flat-square" alt="Terminal"> | Raylib (`make raylib`) <br> <img src="https://img.shields.io/badge/-Raylib-020617?style=flat-square&logo=raylib&logoColor=white"/> |
| :--- | :---: | :---: |
| Adivinhação Solo | ✅ | ✅ — **Operação Resgate** |
| Adivinhação VS | ✅ | ✅ — **Batalha de Sinais** |
...
O jogador tenta descobrir um número inteiro gerado aleatoriamente dentro de um intervalo definido pela dificuldade. Após cada palpite o jogo retorna uma dica direcional: **MAIOR**, **MENOR** ou **ACERTOU**.
...
| Nível | Intervalo | Tentativas máx. |
| :--- | :---: | :---: |
| Fácil | 1 – 10 | 5 |
| Médio | 1 – 50 | 8 |
| Difícil | 1 – 100 | 10 |
...
1. Selecione a dificuldade no menu.
2. Digite um número inteiro a cada rodada.
3. Siga as dicas (MAIOR / MENOR) até acertar ou esgotar as tentativas.
...
$ make run
 ...
 ↑ MAIOR! O número é maior que 25.

 Tentativa 2/8 → Seu palpite: 37
 ↓ MENOR! O número é menor que 37.
...
- Na versão Raylib há um timer de **15 s** por tentativa; o tempo restante contribui para o bônus de pontos.
...
2. Escolha a dificuldade (mesmos intervalos da versão Solo).
...
| **Revelação** | As duas casas são exibidas temporariamente |
| **Verificação** | Par correto → casas marcadas como `acertadas`; par errado → voltam a ficar ocultas |
...
2. O tabuleiro exibe 16 casas numeradas, todas ocultas (`?`).
3. Digite o número de duas casas que deseja revelar.
4. Par correto → `✅` permanece; par errado → casas voltam a `?`.
5. Continue até encontrar todos os **8 pares**.

#### Exemplo de Tabuleiro (Console)
...
> No Raylib, o jogador **clica diretamente** nas casas do tabuleiro. O timer começa em **30 s** e ganha **+10 s** de bônus a cada par acertado.
...
- Na versão Raylib o timer começa em **30 s** e ganha **+10 s** de bônus a cada par acertado.
...
> A `struct JogoMemoria` e toda a lógica de tabuleiro são **idênticas** entre Solo e VS. A gestão de turnos e o registro de ambos os jogadores ficam em `jogar_memoria_vs` (`jogos_extras.c`) e, na versão gráfica, em `ui/frontend.c`.
...
> Cada jogador tem seu próprio timer independente (visível no topo esquerdo e topo direito). O indicador **TURNO** alterna entre os nomes a cada jogada.
...
```bash
 ...
 Segunda casa: 9
 Casa 2: 6  |  Casa 9: 1
 ✖ Não é par. Turno de Astronauta 2.
```
...
1. Leia a fórmula exibida na tela.
2. Decida: verdadeira para todas as combinações (**Tautologia**), falsa para todas (**Contradição**) ou depende dos valores (**Contingência**)?
3. Selecione a resposta antes do timer zerar.
...
```bash
 ...
 Lembretes de lógica:
 TAUTOLOGIA = sempre V  |  CONTRADIÇÃO = sempre F  |  CONTINGÊNCIA = V ou F dependendo dos valores
```
...
- Sistema de **patentes espaciais** (`get_patente_logica`) exibido no resultado final:

| Faixa de pontos | Patente |
| :---: | :--- |
| 0 – 39 | Recruta |
| 40 – 69 | Piloto |
| 70 – 99 | Tenente |
...
| Nível | Questões | Tempo/questão | Operadores presentes |
| :--- | :---: | :---: | :--- |
| Fácil | 8 | 35 s | `~`, `^`, `V` |
| Médio | 10 | 30 s | `~`, `^`, `V`, `->` |
| Difícil | 12 | 25 s | `~`, `^`, `V`, `->`, `<->` |
...
1. Leia a expressão exibida (ex: `P ^ Q V R`).
...
- Pontos por acerto acumulados ao longo das questões.
- Sistema de **patentes espaciais** (`get_patente_precedencia`) baseado na pontuação final (mesmas faixas do Protocolo Lógico).
...
## Diferenças entre Console e Raylib

| Aspecto | Console (`make run`) | Raylib (`make raylib`) |
| :--- | :--- | :--- |
| Interface | ASCII / terminal | Janela gráfica 1200×800 px |
| Entrada | Teclado (stdin) | Mouse + Teclado |
| Timer visual | Não | Barra animada por tela |
...
> Os módulos de lógica (`game/`), histórico (`history/`) e estatísticas (`static/`) são **idênticos** nas duas versões — apenas a camada de apresentação difere (ver [ADR-001](ADR.md)).

---

## Compilação Manual (Console)
...
> Para o schema dos arquivos de dados consulte [schema.md](schema.md).  
> Para as decisões arquiteturais que moldaram esses modos consulte [ADR.md](ADR.md).
